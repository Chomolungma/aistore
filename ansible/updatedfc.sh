...
if [[ -s inventory/new_targets.txt ]]; then parallel-ssh -h inventory/new_targets.txt -i "./mountdfc.sh "$@; fi
parallel-ssh -h inventory/targets.txt -i "mount | grep dfc"
if [[ -s inventory/new_targets.txt ]]; then parallel-ssh -h inventory/new_targets.txt -i "mount | grep dfc"; fi
